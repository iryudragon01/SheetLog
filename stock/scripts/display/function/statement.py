...
def getstatement(start_date):
    ...
    if Income.objects.filter(date_log__gt=start_date):
        ...
        sum_income = Income.objects.filter(date_log__gt=start_date).aggregate(Sum('value'))['value__sum']
    else:
        content['incomes'] = []
    if Expense.objects.filter(date_log__gt=start_date):
        content['expenses'] = Expense.objects.filter(date_log__gt=start_date)
        sum_expense = Expense.objects.filter(date_log__gt=start_date).aggregate(Sum('value'))['value__sum']

    else:
        content['expenses'] = []
    if TempExpense.objects.filter(date_log__gt=start_date):
        content['temps'] = TempExpense.objects.filter(date_log__gt=start_date)
        sum_temp = TempExpense.objects.filter(date_log__gt=start_date).aggregate(Sum('value'))['value__sum']
